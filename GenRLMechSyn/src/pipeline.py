...
class TrainingPipeline:
    def __init__(self, config, project_root):
        ...
        try:
            ...
            # norm_vec 仍然是 3 通道 (a, alpha, offset/d)
            norm_vec = torch.tensor([
                norm_values_dict['a'],
                norm_values_dict['alpha'],
                norm_values_dict['d']  # config key 保持 'd'
    ...
    def _normalize(self, x_tensor):
        """ 将 (B, 5, H, W) 或 (C, 5, H, W) 数据按通道归一化到 [-1, 1]."""
        ...
        joint_type_norm = joint_type_channel  # 已经是 -1/1, 无需操作

        # self.norm_vec (1, 3, 1, 1) 已经在 self.device 上
    ...
    def _unnormalize(self, x_tensor_norm):
        """ 将 (B, 5, H, W) 或 (C, 5, H, W) 数据从 [-1, 1] 按通道恢复."""
        ...
        joint_type_unnorm = joint_type_norm  # 保持 -1/1
    ...
    def load_replay_buffer(self):
        """
        从 augmented_dataset 加载经验 (8列稀疏 -> 5通道稠密).
        """
        ...
        for entry in manifest:
            ...
            try:
                ...
                with np.load(npz_path) as data:
                    ...
                    edge_list_array = data['edge_list_array']  # (N, 8)

                    # 像 Dataloader 一样动态构建稠密张量
                    max_nodes = self.config['data']['max_nodes']
                    num_features = self.config['diffusion_model']['in_channels']  # 5
                    ...
                    tensor_norm = self._normalize(tensor_unnorm_torch)  # 使用 pipeline 的 5-ch normalizer
    ...
    def _generate_and_augment(self, cycle_num, total_cycles):
        ...
        # --- 接收两个输出 ---
        # 1. new_mech_tensors_unnorm_numpy: list of (H, W, 5) numpy 数组 (已后处理)
        # 2. batch_x0_norm_pure: (B, 5, H, W) torch 张量 (未后处理, 已归一化)
        ...
        # --- 使用 enumerate 获取索引 ---
        for i, tensor_unnorm_numpy in enumerate(new_mech_tensors_unnorm_numpy):  # (H, W, 5)
            ...
            #    我们不再需要 _normalize()
            tensor_torch_norm_pure = batch_x0_norm_pure[i].to(self.device)  # (5, H, W)
    ...
    def _train_dit_model(self, cycle_num):
        ...
        for epoch in range(num_epochs):
            total_loss = 0.0
            # Dataloader 返回 (x_start(B, 5, H, W), y_label)
            for x_start, y_labels in current_dataloader:
                ...
                x_start_norm = self.diffusion_model._normalize(x_start)
                noise = torch.randn_like(x_start_norm)  # (B, 5, H, W)
    ...
    def _warmup_dit(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            total_loss = 0.0
            # Dataloader 返回 (x_start(B, 5, H, W), y_labels)
    ...
    def _warmup_rl_agent(self, num_epochs):
        ...
        for x_start_batch, y_labels_batch in temp_loader:
            for i in range(x_start_batch.size(0)):
                x_start_unnorm = x_start_batch[i]  # (5, H, W)
                y_label = y_labels_batch[i]

                x_start_norm = self._normalize(x_start_unnorm)  # (5, H, W)
