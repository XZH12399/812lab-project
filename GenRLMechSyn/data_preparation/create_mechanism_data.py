...
# --- 3. 保存函数 ---
def save_mechanism_sparse(edge_list_data, joint_types_dict, filename):
    """
    将 edge_list (6列) 和 joint_types (1D 数组) 保存到 .npz 文件.
    """
    ...
    try:
        ...
        # --- 2. 保存 joint_types (1D 数组) ---
        # (假设 joint_types_dict 的键总是 0, 1, 2, ...)
        joint_types_array = np.array([joint_types_dict[j] for j in sorted(joint_types_dict.keys())], dtype=np.float32)
